...
var par_reg = doc.paragraphStyles.itemByName("Text) Body / Regular / 6pt");
var par_bld = doc.paragraphStyles.itemByName("Text) Body / Bold / 6pt");

if (par_reg.isValid == true && par_bld.isValid == true) {
...
for (var i = 0; i < sel.cells.length; i++) {
  if (sel.cells[i].texts[0].contents == "") {
    sel.cells[i].contents = "-";
  }
  var while_count = 0;
  while (sel.cells[i].overflows == true && while_count < 20) {
    sel.cells[i].width++;
    while_count++;
  }
}
...
if (tab_size == 1 && tab_width != "") {
  var tab_array = tab_width.split(",");
  if (tab_array.length > 1) {
    for (var i = 0; i < tab_array.length; i++) {
      try {
        sel.columns[i].width = tab_array[i];
      } catch (e) {}
    }
  } else {
    sel.columns[0].width = tab_array[0];
    for (var i = 1; i < sel.columns.length; i++) {
      sel.columns[i].width = (88 - tab_width) / (sel.columns.length - 1);
    }
  }
}

var cell_inset = 0;
var radio_table_inset;
try {
  cell_inset = radio_inset_disable.value;
  radio_table_inset = [
    edittext_left.text,
    edittext_right.text,
    edittext_top.text,
    edittext_bottom.text,
  ];
} catch (e) {}
...
if (tab_size == 0) {
  try {
    sel.cells[0].insertionPoints[0].parentTextFrames[0].fit(
      FitOptions.FRAME_TO_CONTENT
    );
    sel.cells[0].insertionPoints[0].parentTextFrames[0].textFramePreferences.autoSizingReferencePoint =
      AutoSizingReferenceEnum.TOP_LEFT_POINT;
    sel.cells[0].insertionPoints[0].parentTextFrames[0].textFramePreferences.autoSizingType =
      AutoSizingTypeEnum.HEIGHT_ONLY;
    doc.align(
      sel.cells[0].insertionPoints[0].parentTextFrames[0],
