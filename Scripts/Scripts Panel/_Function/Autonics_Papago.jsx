...
try {
  for (i = 0; i < sel.tables.everyItem().cells.length; i++) {
    ...
    _contents += String("\r" + sel.tables.everyItem().cells[i].contents); //다음 셀 내용 넘어갈 때마다 줄바꿈 추가 (줄바꿈 유니코드 000A -> 웹상 유니코드 %0A)
  }
} catch (e) {}
...
for (i = 0; i < _contents.length; i++) {
  ...
  if (_contents[i] == "&") {
    ...
    translate += "%25amp"; //해당 글자는 UTF-8로 변환
  } else if (_contents[i] == "") {
    //텍스트 프레임 내 테이블 같은 고정 개체 일 경우
    translate += ""; //없앰
  } else {
    // 그 외
    translate += encodeURI(_contents[i]); //UTF-8로 변환
  }
}

var _link = "https://papago.naver.com/?sk=ko&tk=en&st="; //파파고 링크 해당 링크 다음에 문자열 추가하면 문자열 번역함
