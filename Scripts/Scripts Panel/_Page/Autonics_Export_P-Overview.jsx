...
//==========단 좌표 설정==========
var section_1st = [15, 0]; //1번째 단 x, y 좌표 값
var section_2nd = [108, 0]; //2번째 단 x, y 좌표 값
...
if (PDF_error(doc) == true) {
  ...
  with (new_doc) {
    ...
    pages[0].appliedMaster = new_doc.masterSpreads.itemByName("C-Catalog");
  }
}
...
if (page_range == 0) {
  ...
  var count = 2;
  for (var i = 2; i < doc.pages.length; i++) {
    duplicate_data();
    count++;
  }
} else {
  ...
  doc.pages[page_range].pageItems
    .everyItem()
    .duplicate(new_doc.pages[0], section_2nd); //활성화 되어있는 문서의 페이지에 해당하는 모든 개체를 새로운 문서의 section_2nd 좌표에 붙여넣기 함
  var count = 2;
  for (var i = page_range + 1; i < doc.pages.length; i++) {
    duplicate_data();
    count++;
  }
}
...
try {
  new_doc.conditions.itemByName("제품 개요서").visible = true;
} catch (e) {}
...
if (doc.pages.lastItem().bounds[3] < 90) {
...
function new_folder(doc_format) {
  if (create_folder == 1) {
    var lang = language == "K-Catalog" ? "국문" : "영문";
...
if (view == false) {
  ...
  var name = new File(
    save_loc + "/" + folder_dir + doc_name + "_CATALOG_W" + ".pdf"
  ); //새파일 만들기 (폴더 경로/파일명_CATALOG.pdf)
  if (export_subdir == true) {
    name = new File(
      doc.filePath + "/" + folder_dir + doc_name + "_CATALOG_W" + ".pdf"
    ); //새파일 만들기 (폴더 경로/파일명_CATALOG.pdf)
  }
  new_doc.exportFile(
  ...
  if (export_idml == 1) {
    var name = new File(
      save_loc + "/" + folder_dir + doc_name + "_CATALOG" + ".idml"
    ); //새파일 만들기 (폴더 경로/파일명_IDML.idml)
    if (export_subdir == true) {
      name = new File(
        doc.filePath + "/" + folder_dir + doc_name + "_CATALOG" + ".idml"
...
function cover_move() {
  ...
  for (i = 0; i < new_doc.pages[0].pageItems.length; i++) {
    if (
      new_doc.pages[0].pageItems[i].geometricBounds[1] > 105 &&
      new_doc.pages[0].pageItems[i].itemLayer.name != "Note"
    ) {
